...
def draw_topper(draw: ImageDraw.ImageDraw, kind: str, center: tuple[int, int], scale: float = 1.0) -> None:
    ...
    if kind == "strawberry":
        ...
        draw.polygon([(cx, cy - 38 * scale), (cx + 22 * scale, cy - 20 * scale), (cx + 8 * scale, cy - 8 * scale)], fill=rgba(PALETTE["green"], 255))
        draw.polygon([(cx, cy - 38 * scale), (cx - 22 * scale, cy - 20 * scale), (cx - 8 * scale, cy - 8 * scale)], fill=rgba(PALETTE["green"], 255))
        for dx in (-12, 0, 12):
            for dy in (-4, 12):
        ...
        for offset_x, offset_y in [(-30, 6), (22, -18), (36, 10)]:
            ...
            draw.polygon(
                [
                    ...
                    (px + 10 * scale, py - 2 * scale),
                    (px + 5 * scale, py + 2 * scale),
                    (px + 7 * scale, py + 9 * scale),
                    (px, py + 5 * scale),
                    (px - 7 * scale, py + 9 * scale),
                    (px - 5 * scale, py + 2 * scale),
                    (px - 10 * scale, py - 2 * scale),
...
def draw_piping_bag(draw: ImageDraw.ImageDraw, x: int, y: int, scale: float) -> None:
    draw.polygon([(x, y), (x + 164 * scale, y + 36 * scale), (x + 74 * scale, y + 204 * scale)], fill=rgba(PALETTE["warm_white"], 240), outline=rgba(PALETTE["blue"], 200))
    draw.polygon([(x + 36 * scale, y + 28 * scale), (x + 144 * scale, y + 54 * scale), (x + 86 * scale, y + 164 * scale)], fill=rgba(PALETTE["blue_soft"], 170))
